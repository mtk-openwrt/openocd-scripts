...
1. Connect to MT7981 board using mt7981.cfg
...
5. Switch CPU to AArch64 state
   ...
   After resume, there may be an error "mt7981.cpu0: Timeout waiting for resumex", just ignore it
...
make -f Makefile CROSS_COMPILE=<toolchain-prefix> LOG_LEVEL=30 PLAT=mt7981 <dram-config> BOOT_DEVICE=ram RAM_BOOT_DEBUGGER_HOOK=1 bl2

<dram-config>:
For bl2-ddr3-qfn: DRAM_USE_DDR4=0 BOARD_QFN=1
For bl2-ddr3: DRAM_USE_DDR4=0 BOARD_BGA=1
For bl2-ddr4: DRAM_USE_DDR4=1 BOARD_BGA=1

Final binary is build/mt7981/release/bl2.bin